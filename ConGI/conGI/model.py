...
class TrainerSpaCLR:
    def __init__(self, args, network, optimizer, log_dir, device='cuda', manual_clusters=7):
        ...
        elif args.dataset == 'DLPFC':
            adata = sc.read_h5ad(os.path.join(args.path, args.name))
        ...
        try:
            clusters = self.label.unique()
            if np.NaN in clusters:
