...
def parameter_setting():
    ...
    # preprocess
    parser.add_argument('--dataset', type=str, default="Hudson")
    parser.add_argument('--path', type=str, default=r"D:\Pitagoras\Spatia_seq\Dataset\ARTICULOS\DLPFC")
    parser.add_argument("--gene_preprocess", choices=("pca", "hvg"), default="pca")
    parser.add_argument("--n_gene", choices=(300, 1000), default=1000)
...
def build_her2st_data(path, name, size=112):
    ...
    # Se puede simplificar con un zip()
    x = df_pos['x'].values
    y = df_pos['y'].values
    ids = []
...
def refine(sample_id, pred, dis, num_nbs):
    ...
    refined_pred = pd.DataFrame({"pred": refined_pred}, index=sample_id)
    return refined_pred
...
def get_predicted_results(dataset, name, path, z):
    ...
    if dataset=="SpatialLIBD":
        ...
        adata = load_ST_file(os.path.join(path, name, 'ST'))
        adj_2d = calculate_adj_matrix(x=adata.obs["array_row"].tolist(), y=adata.obs["array_col"].tolist(),
                                histology=False)

        raw_preds = eval_mclust_ari(1, z, 7)
        ...
        adata = sc.read_h5ad(os.path.join(path, name))
        adj_2d = calculate_adj_matrix(x=adata.obs["array_row"].tolist(), y=adata.obs["array_col"].tolist(),
                                      histology=False)
        
        d = adata.obs['Region'].dropna()
        try: 
            clusters = d.unique()
        ...
        raw_preds = eval_mclust_ari(1, z, n_clusters)
        ...
        shared_idx = refined_preds.index.intersection(d.index)
        
        d = d.loc[shared_idx]
        refined_preds = refined_preds.loc[shared_idx]
        
        label = pd.Categorical(d).codes

        ari = adjusted_rand_score(label, refined_preds['pred'].tolist())
