...
parser.add_argument('--path', type=str, default=r"D:\Pitagoras\Spatia_seq\Dataset\her2st-master")
...
args = parser.parse_args(args=['--epochs', '5', '--name', 'A1'])
print(args)
...
def train():
    ...
    # Podríamos configurar el optimizar e ir cambiando el learning rate.
...
def main():
    ...
    metadata_path = os.path.join(args.path, 'metadata')
    metadata = [x for x in os.listdir(metadata_path) if args.name in x]
    # adata = load_ST_file(os.path.join(args.path, args.name))
    df_meta = pd.read_csv(os.path.join(metadata_path, metadata[0]), sep='\t')
    
    label = pd.Categorical(df_meta['label']).codes
    # adata = adata[label != -1]
    # label = label[label != -1]
    # n_clusters = label.max()+1

    pred = pd.read_csv(f'output/{args.name}_pred.csv')['cluster_labels']
    adata.obs['label'] = df_meta['label']
