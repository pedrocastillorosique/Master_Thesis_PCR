...
dataPath = "D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-67-3T-6/outs"
## Obtener matriz de spot/célula y la proporción de cada tipo celular
deconvolution = readRDS("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Resultados_RCTD/myRCTD_67.rds")
...
seurat_object@meta.data <- cbind(seurat_object@meta.data, cell_type_matrix)
...
bayes_object <- readRDS("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Objetos_Bayes_VDJ/seurat_67_VDJ_with_bayesCluster_fixed.rds")
...
AP_BayesSpace <- data.frame(
  ...
  Category = c("Borde","Tumor", "Tumor", "Estroma", "Tumor", "Estroma", "Estroma")
)
```
...
SpatialFeaturePlot(seurat_object, features = "T_cell", pt.size.factor = 2) + ggtitle("T cell proportion") +
  theme_minimal()
```

```{r}
SpatialFeaturePlot(seurat_object, features = "B_cell", pt.size.factor = 2) + ggtitle("B cell proportion") +
  theme_minimal()
```
...
prop_df <- as.matrix(cell_type_matrix)
...
prop_df_norm$Spot <- rownames(prop_df)
prop_df_norm$Spot_cluster <- seurat_object$Spot_cluster[match(colnames(seurat_object), prop_df_norm$Spot)]
...
prop_df_norm$BayesSpace <- seurat_object$BayesSpace[match(rownames(prop_df), colnames(seurat_object))]
...
```{r}
library("readxl")
spatial_matrix <- read_xlsx("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Clonotipos_SPATIAL/GIN_67_spatial_clonotype_counts_filtered.xlsx")
...
```{r}
SpatialFeaturePlot(seurat_object, features = "clone22", pt.size.factor = 3) &
...
```{r}
stKeep_object = readRDS("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-67-3T-6/outs/stKeep/stKeep_GIN-67.rds")
...
AP_stKeep <- data.frame(
  ...
  Category = c("Tumor","Tumor", "Tumor", "Estroma", "Estroma", "Borde", "Borde")
)
```
...
prop_df_norm$stKeep_cluster <- seurat_object$stKeep_cluster[match(rownames(prop_df), colnames(seurat_object))]
...
```{r}
graphST_object = read_h5ad("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-67-3T-6/outs/graphST/GIN-67-3T-6.h5ad")  
...
# Hacemos una relación entre la anotación de AP y los clusters. 
AP_graphST <- data.frame(graphST = as.character(1:7),   
                        Category = c("Borde","Tumor", "Tumor", "Estroma",
                                     "Tumor", "Tumor", "Estroma") )
```
