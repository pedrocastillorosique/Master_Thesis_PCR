...
dataPath = "D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-63-3T-4/outs"
## Obtener matriz de spot/célula y la proporción de cada tipo celular
deconvolution = readRDS("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Resultados_RCTD/myRCTD_63.rds")
...
anno$Layer[anno$Layer == "cluster3"] <- "Border"
...
```{r}
SpatialPlot(seurat_object, group.by="Spot_cluster") + ggtitle("Anotación AP") + theme_minimal() + guides(fill = guide_legend(override.aes = list(size = 3)))
```
...
bayes_object <- readRDS("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Objetos_Bayes_VDJ/seurat_63_VDJ_with_bayesCluster_fixed.rds")
...
AP_BayesSpace <- data.frame(
  BayesSpace = as.character(1:8),
  Category = c("Estroma", "Tumor", "Tumor", "Border", "Border", "Tumor", "Tumor", "Border")
)
```

```{r}

SpatialPlot(seurat_object, group.by="BayesSpace") + ggtitle("Spatial Cluster") + theme_minimal() + guides(fill = guide_legend(override.aes = list(size = 3))) + scale_fill_manual(values = my_colors)
```
...
SpatialFeaturePlot(seurat_object, features = "T-cell") + ggtitle("T cell proportion") +
  theme_minimal()
```

```{r}
SpatialFeaturePlot(seurat_object, features = "B-cell") + ggtitle("B cell proportion") +
  theme_minimal()
```

```{r}
SpatialFeaturePlot(seurat_object, features = "Malignant") + ggtitle("Malignant proportion") +
  theme_minimal()
```
...
summary_df <- summary_df[summary_df$Spot_cluster != "cluster3", ]
...
library("readxl")
spatial_matrix <- read_xlsx("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Clonotipos_SPATIAL/GIN_63_spatial_clonotype_counts_filtered.xlsx")
...
```{r}
SpatialFeaturePlot(seurat_object, features = c("clone8"), pt.size.factor = 3) &
  scale_fill_gradient2(low = "white", mid = "blue", high = "red", midpoint = .5) &   scale_alpha_continuous(range=c(0,2)) 
```

```{r}
SpatialFeaturePlot(seurat_object, features = "clone14", pt.size.factor = 3) &
  scale_fill_gradient2(low = "white", mid = "blue", high = "red", midpoint = 0.5) &   scale_alpha_continuous(range=c(0,1)) 
```

```{r}
SpatialFeaturePlot(seurat_object, features = "clone7", pt.size.factor = 3) &
  scale_fill_gradient2(low = "white", mid = "blue", high = "red", midpoint = 0.5) &   scale_alpha_continuous(range=c(0,2)) 
```

```{r}

SpatialFeaturePlot(seurat_object, features = "clone0", pt.size.factor = 3) &
  scale_fill_gradient2(low = "white", mid = "blue", high = "red", midpoint = 0.5) &   scale_alpha_continuous(range=c(0,25))
```
...
```{r}
stKeep_object = read_h5ad("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-63-3T-4/outs/stKeep/stKeep_GIN-63.rds")

stKeep_cluster = stKeep_object$obs$mclust[match(colnames(seurat_object), rownames(stKeep_object))]

# seurat_object$BayesSpace <- as.numeric(as.character(BayesSpace))
seurat_object$stKeep_cluster <- stKeep_cluster

# Hacemos una relación entre la anotación de AP y los clusters.
AP_stKeep <- data.frame(
  stKeep_cluster = as.character(1:8),
  Category = c("Estroma", "Tumor", "Tumor", "Border", "Border", "Tumor", "Tumor", "Border")
)
```

```{r}
SpatialDimPlot(seurat_object, group.by = "stKeep_cluster", label.size = 3) + ggtitle("Spatial Cluster") + theme_minimal() + guides(fill = guide_legend(override.aes = list(size = 3))) + scale_fill_manual(values = my_colors)
```
...
```{r}
stKeep_object = read_h5ad("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-63-3T-4/outs/graphST/GIN-63-3T-4.h5ad")  

graphST = stKeep_object$obs$mclust[match(colnames(seurat_object), rownames(stKeep_object))]  

#seurat_object$BayesSpace <- as.numeric(as.character(BayesSpace))
seurat_object$graphST <- graphST

# Hacemos una relación entre la anotación de AP y los clusters. 
SpatialDimPlot(seurat_object, group.by = "graphST", label.size = 3) + ggtitle("Spatial Cluster") + theme_minimal() + guides(fill = guide_legend(override.aes = list(size = 3))) + scale_fill_manual(values = my_colors)

AP_graphST <- data.frame(graphST = as.character(1:8), Category = c("Borde", "Estroma", "Tumor", "Tumor", "Tumor", "Tumor", "Borde", "Borde") )
```
...
ggplot(AP_graphST_df, 
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1),
        ...
        panel.grid = element_blank()   ) +
  labs(title = "Distribución de clones por clúster") +    
  theme(strip.text = ggtext::element_markdown(face = "bold", size = 7),
