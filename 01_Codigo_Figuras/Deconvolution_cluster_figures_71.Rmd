...
dataPath = "D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-71-3T-1/outs"
## Obtener matriz de spot/célula y la proporción de cada tipo celular
deconvolution = readRDS("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Resultados_RCTD/myRCTD_71.rds")
...
bayes_object <- readRDS("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Objetos_Bayes_VDJ/seurat_71_VDJ_with_bayesCluster_fixed.rds")
...
AP_BayesSpace <- data.frame(
  ...
  Category = c("Estroma","Tumor", "Estroma", "Estroma", "Tumor", "Borde", "Estroma", "Borde")
)
```
...
```{r}
library("readxl")
spatial_matrix <- read_xlsx("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/Datos_Pedro/Clonotipos_SPATIAL/GIN_71_spatial_clonotype_counts_filtered.xlsx")
...
```{r}
stKeep_object = readRDS("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-71-3T-1/outs/stKeep/stKeep_GIN-71.rds")
...
AP_stKeep <- data.frame(
  ...
  Category = c("Estroma","Tumor", "Borde", "Estroma", "Estroma", "Tumor", "Borde")
)
```
...
graphST_object = read_h5ad("D:/Pitagoras/Spatia_seq/Dataset/PITAGORAS/GIN-71-3T-1/outs/graphST/GIN-71-3T-1.h5ad")  
...
# Hacemos una relación entre la anotación de AP y los clusters. 
AP_graphST <- data.frame(graphST = as.character(1:8),   
                        Category = c("Borde","Tumor", "Estroma", "Estroma",
                                     "Estroma", "Tumor", "Estroma", "Borde") )
```
...
umap_coords <- umap(graphST_object$obsm$emb_pca, n_components = dim(graphST_object$obsm$emb_pca)[2], n_neighbors = 50)

# Convertir a data.frame para plotear
df_umap <- as.data.frame(umap_coords)
colnames(df_umap) <- c("UMAP1", "UMAP2")
df_umap$cluster <- graphST_object$obs$mclust

# Añadir variable de interés para colorear (por ejemplo, mclust)
ggplot(df_umap, aes(x = UMAP1, y = UMAP2, color = as.factor(cluster))) +
  geom_point(size = 1.5, alpha = 0.8) +
  scale_color_manual(values = my_colors) +   # usa tu paleta personalizada
  labs(title = "", color = NULL) +  # sin título de leyenda
  theme_minimal() +
  theme(
    panel.grid = element_blank()
  )
...
tabla[, 2:ncol(tabla)] <- (tabla[, 2:ncol(tabla)] > 0) * 1
...
summary_tabla_wide_df <- summary_tabla_long_df %>%
  pivot_wider(
    names_from = graphST,
    values_from = counts,
    values_fill = 0
...
write.csv(summary_tabla_wide_df, "G:/Mi unidad/TFM Pedro/Datos/Clonotipos_SPATIAL/GIN_71_tabla_counts_summary.csv", row.names = TRUE)
