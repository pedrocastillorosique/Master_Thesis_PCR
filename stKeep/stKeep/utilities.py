...
def parameter_setting():
	...
	parser.add_argument('--inputPath',   '-IP', type = str,    help='data directory')	
	...
	parser.add_argument('--annotation_method', '-am', type=str, default = 'Qupath', help='annotation method')
	parser.add_argument('--annoFile', '-aFile', type=str, default = 'Image_cell_segmentation.txt', help='annotation file')
	...
	parser.add_argument('--hgvsFile', type=str, default = 'highly_variable_genes.txt', help='highly variable genes')
...
def load_ccc_data( args ):
	...
	spots_ligand.fillna(0, inplace=True)
	...
	spots_recep.fillna(0, inplace=True)
...
def get_cell_positive_pairs(adata, args):
	...
	cell_cell_adj = np.zeros((len(adata), len(adata)), dtype = int)
...
def find_hvgs(args, adata):
	'''Function to find high variable genes'''

	save_path = args.inputPath + "highly_variable_genes.txt"
	bdata = adata.copy()
	
	sc.pp.normalize_total(bdata, inplace=True, exclude_highly_expressed= False)
	sc.pp.log1p(bdata)
	sc.pp.highly_variable_genes(bdata, n_top_genes=2000, flavor="seurat_v3")

	hvgs = bdata.var[bdata.var["highly_variable"]].index.tolist()
