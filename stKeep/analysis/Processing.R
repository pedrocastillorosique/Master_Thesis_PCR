...
Preprocess_CCC_model <- function(basePath = "../test_data/DLPFC_151507/", utili_path = "../utilities/", LRP_data = "Uninon_Ligand_receptors.RData"){
  ...
  anno            = read.table(paste0(basePath,"Image_cell_segmentation.txt"), header = T, row.names = 1 )
...
Cell_modules <- function(basePath, nCluster = 7, save_path = NULL, pdf_file = NULL ){
...
Gene_modules <- function(Cell_obj, Gene_rep, nCluster = 7, save_path = NULL, pdf_file = NULL ){
  ...
  expr_matrix <- as.matrix(Gene_obj@assays$RNA$scale.data)
  gene_clusters <- Idents(Gene_obj)

  inter_genes <- list()

  for (cl in levels(gene_clusters)) {
    genes_in_cl <- names(gene_clusters[gene_clusters == cl])
    # Submatriz de expresión solo para genes de este clúster
    sub_expr <- expr_matrix[, genes_in_cl , drop = FALSE]
    # Calcular expresión media por gen (fila)
    gene_means <- colMeans(sub_expr)
    # Seleccionar top 3
    top_genes <- names(sort(gene_means, decreasing = TRUE))[1:3]
    inter_genes[[cl]] <- top_genes
  }
  inter_genes  <- unlist(inter_genes, use.names = FALSE) 
