...
def calculate_adj_matrix(x, y, x_pixel=None, y_pixel=None, image=None, beta=49, alpha=1, histology=True, path = None):
	#x,y,x_pixel, y_pixel are lists
	if histology:
		assert (x_pixel is not None) & (x_pixel is not None) & (image is not None)
		assert (len(x)==len(x_pixel)) & (len(y)==len(y_pixel))
		print("Calculateing adj matrix using histology image...")
		#beta to control the range of neighbourhood when calculate grey value for one spot
		#alpha to control the color scale
		beta_half=round(beta/2)
		# Create new folder to save tiles
		os.makedirs(path + '/tiles', exist_ok=True)
		g=[]
		for i in range(len(x_pixel)):
			max_x=image.shape[0]
			max_y=image.shape[1]
			nbs=image[int(max(0,x_pixel[i]-beta_half)):int(min(max_x,x_pixel[i]+beta_half+1)),
			 		  int(max(0,y_pixel[i]-beta_half)):int(min(max_y,y_pixel[i]+beta_half+1))]
			plt.imsave(path + f'/tiles/{str(y_pixel[i])}' + "-" + f'{str(x_pixel[i])}.png', nbs)
			g.append(np.mean(np.mean(nbs,axis=0),axis=0))
		c0, c1, c2=[], [], []
		for i in g:
			c0.append(i[0])
